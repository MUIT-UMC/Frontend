    ...
    return (
        <Container>
            <TheaterInfo>
                ...
                <h3 className="title-B">{props?.data?.placeName}</h3>
                <p className="body-M-500">{props?.data?.address}</p>

                <Text>
                    <p className="body-M-400">현재 공연</p>
                    <span className="ShowTitle"
                    onClick={()=>{navigate(`/detail/${props?.data?.musicalId}`)}}>
                    <img src={ChevronRight} />
                    <MusicalLink>&nbsp; {props?.data?.musicalName}</MusicalLink>
                    </span>
                </Text>
